...
def update_mart_d_tables(ti):
    ...
    #d_calendar
    cur.execute("""TRUNCATE mart.d_calendar CASCADE;
    with all_dates as (
    ...
    select 
        distinct ROW_NUMBER () OVER (
            ORDER BY date_time) as date_id
        ,date_time::date as fact_date
    ...
    #d_customer
    cur.execute("""TRUNCATE mart.d_customer CASCADE;
    ...
    #d_item
    cur.execute("""TRUNCATE mart.d_item CASCADE;
...
def update_mart_f_tables(ti):
    ...
    group by dc.date_id, ual.action_id;""")
    conn.commit()
